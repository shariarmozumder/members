<?php
    ...
    if(isset($_POST['login'])) {
        ...
        $sql = "SELECT * FROM schools WHERE cnic='".$username."' AND password = '".$password."'";
        $q = $conn->query($sql);
...
<html>
    ...
    <form action="login.php" method="post">
        ...
        <input type="text" class="form-control"  data-inputmask="'mask': '99999-9999999-9'"  placeholder="XXXXX-XXXXXXX-X"  name="username" required="" " />
        <script>
            $(":input").inputmask();
        </script>
        <label for="password">
